(function(){
	...
	function renderStock(data){
		...
		let mostrarBajo = bajoUl.getAttribute('data-ver-mas') === 'true' ? bajo : bajo.slice(0,5);
		let mostrarIdeal = idealUl.getAttribute('data-ver-mas') === 'true' ? ideal : ideal.slice(0,5);
	...
	document.addEventListener('DOMContentLoaded', async ()=>{
		...
		// Listeners para paginación con flechas y puntitos
		document.getElementById('stock-bajo-list').addEventListener('click', function(e){
			if(e.target && e.target.id === 'btn-bajo-prev' && __stockPage.bajo > 0){
				__stockPage.bajo--;
				renderStock(__lastStockData);
			}
			if(e.target && e.target.id === 'btn-bajo-next'){
				const bajo = (__lastStockData && Array.isArray(__lastStockData.bajo)) ? __lastStockData.bajo : [];
		...
		document.getElementById('stock-ideal-list').addEventListener('click', function(e){
			if(e.target && e.target.id === 'btn-ideal-prev' && __stockPage.ideal > 0){
				__stockPage.ideal--;
				renderStock(__lastStockData);
			}
			if(e.target && e.target.id === 'btn-ideal-next'){
				const ideal = (__lastStockData && Array.isArray(__lastStockData.ideal)) ? __lastStockData.ideal : [];
